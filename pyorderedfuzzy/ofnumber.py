...
class Branch(object):
    ...
    @staticmethod
    def init_gauss(m, s, dim=11, x0=10):
        assert (dim > 1), 'dim must be > 1'
        branch = Branch(np.zeros(dim))
    ...
    @staticmethod
    def init_from_scalar(s, dim=11):
        assert isscalar(s), 's must be scalar'
        return Branch(np.ones(dim) * s)
    ...
    def __add__(self, right):
        ...
        if isinstance(right, Branch):
            assert (res.dim == right.dim), 'self.dim and right.dim must be equal'
    ...
    def __radd__(self, left):
        ...
        if isinstance(left, Branch):
            assert (res.dim == left.dim), 'self.dim and left.dim must be equal'
...
class OFNumber(object):
    ...
    # initialize branch of gaussian type y = s * sqrt(-2 * ln(x)) + m
    @staticmethod
    def init_gaussian(mf, sf, mg, sg, dim=11, x0=10):
        assert (dim > 1), 'dim must be > 1'
        branch_f = Branch.init_gauss(mf, sf, dim=dim, x0=x0)
        branch_g = Branch.init_gauss(mg, sg, dim=dim, x0=x0)
        return OFNumber(branch_f, branch_g)
    ...
    def init_from_scalar(s, dim=11):
        assert isscalar(s), 's must be scalar'
        branch_f = Branch.init_from_scalar(s, dim=dim)
        branch_g = Branch.init_from_scalar(s, dim=dim)
        return OFNumber(branch_f, branch_g)
...
def fabs(ofn):
    res = ofn.copy()
    res.branch_f = Branch(np.abs(res.branch_f.fvalue_y))
    res.branch_g = Branch(np.abs(res.branch_g.fvalue_y))
    return res
...
def flog(ofn):
    ...
    res = ofn.copy()
    res.branch_f = Branch(np.log(res.branch_f.fvalue_y))
    res.branch_g = Branch(np.log(res.branch_g.fvalue_y))
    return res


def fexp(ofn):
    res = ofn.copy()
    res.branch_f = Branch(np.exp(res.branch_f.fvalue_y))
    res.branch_g = Branch(np.exp(res.branch_g.fvalue_y))
    return res
...
def fpower(ofn, p):
    ...
    ofn_1 = OFNumber.init_from_scalar(ofn, dim=p.branch_f.dim) if isscalar(ofn) else ofn.copy()
    ofn_2 = OFNumber.init_from_scalar(p, dim=ofn.branch_f.dim) if isscalar(p) else p.copy()
    
    res = ofn_1.copy()
    res.branch_f = Branch(np.power(ofn_1.branch_f.fvalue_y, ofn_2.branch_f.fvalue_y))
    res.branch_g = Branch(np.power(ofn_1.branch_g.fvalue_y, ofn_2.branch_g.fvalue_y))
    return res
...
def fmax(ofn1, ofn2):
    ...
    ofn_1 = OFNumber.init_from_scalar(ofn1, dim=ofn2.branch_f.dim) if isscalar(ofn1) else ofn1.copy()
    ofn_2 = OFNumber.init_from_scalar(ofn2, dim=ofn1.branch_f.dim) if isscalar(ofn2) else ofn2.copy()

    res = ofn_1.copy()
    res.branch_f = Branch(np.max([ofn_1.branch_f.fvalue_y, ofn_2.branch_f.fvalue_y], axis=0))
    res.branch_g = Branch(np.max([ofn_1.branch_g.fvalue_y, ofn_2.branch_g.fvalue_y], axis=0))
    return res
...
def fmin(ofn1, ofn2):
    ...
    res = ofn_1.copy()
    res.branch_f = Branch(np.min([ofn_1.branch_f.fvalue_y, ofn_2.branch_f.fvalue_y], axis=0))
    res.branch_g = Branch(np.min([ofn_1.branch_g.fvalue_y, ofn_2.branch_g.fvalue_y], axis=0))
    return res
