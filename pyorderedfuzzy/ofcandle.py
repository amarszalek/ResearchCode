...
def check_inputs(data, params):
    ...
    params_fields = ['wtype', 'param_s', 'param_c', 'dim']
...
def compute_linear(price, volume, spread, params):
    ...
    if params['order'](price):
        ...
        ofc = ofn.OFNumber.init_trapezoid_x0x1(fx0, fx1, gx0, gx1, dim=params['dim'])
    else:
...
def compute_gauss(price, volume, spread, params):
    ...
    if params['order'](price):
        ...
        ofc = ofn.OFNumber.init_gaussian(mf, sf, mg, sg, dim=params['dim'])
    else:
        mf = s2
